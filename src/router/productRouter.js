...
productRouter.get('/filter/product', async (req, res) => {
  try {
    ...
    const product = await Product.find({ category }).skip(skip).limit(LIMIT);
...
productRouter.post('/product/add', async (req, res) => {
  try {
    const { productName, price, description, stock, color, category } =
      req.body;
    ...
    if (
      ...
      description.descriptionData.length === 0
    ) {
      throw new Error(
        'description details must be provided and should be an array'
      );
    }
    ...
    for (const col of color) {
      if (!col.colorName || !col.mainImageUrl || !col.hoverImageUrl) {
        throw new Error(
          "Each color must include 'colorName', 'mainImageUrl', and 'hoverImageUrl'"
        );
      }
    }
